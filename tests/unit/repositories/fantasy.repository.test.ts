...
describe("FantasyRepository", () => {
  ...
  it('should have getTournament method with correct signature', () => {
    assert.strictEqual(typeof repository.getTournament, 'function');
    
    // Test that method expects a string parameter and returns a Promise
    const result = repository.getTournament('test');
  ...
  it('should inherit from SportsRepository', () => {
  ...
  it('should handle string webname parameter', () => {
    ...
    validWebnames.forEach(webname => {
      const result = repository.getTournament(webname);
      assert.ok(result instanceof Promise);
    });
  });
  ...
  it('should handle empty webname gracefully', () => {
    ...
    const result = repository.getTournament('');
    assert.ok(result instanceof Promise);
