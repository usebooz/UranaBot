...
- `npm run telegram:mtcute:connectivity` probes mtcute MTProto connectivity to Telegram test servers
...
Experimental Telegram MTProto connectivity probe:

- `scripts/telegram-mtcute-connectivity.js` uses an MTProto user session via `@mtcute/node`
- it is intended to verify Telegram test-server connectivity before adding e2e tests
- it does not test bot commands; command e2e tests should live under `tests/`
